...
-- Tiempo de generación: 06-08-2014 a las 20:50:28
...
VALUES(@_codpersona,_email,_contraseña,'2',now(),'I');

END IF;
END$$

CREATE DEFINER=`canch_admin`@`%` PROCEDURE `USP_GEN_S_CANCHAS`(IN `_accion` VARCHAR(50), IN `_criterio` VARCHAR(100), IN `_iddepa` CHAR(5), IN `_idprov` CHAR(5), IN `_iddis` CHAR(5))
BEGIN
...
can.nCanVisitas,can.nCanEnlace
...
can.cCanFotoPortada,can.nCanEnlace
FROM canchas can
LEFT JOIN detalle_canchas vdir ON can.nCanID = vdir.nCanID AND vdir.nParID = 12 AND vdir.nPcaID = 11
LEFT JOIN detalle_canchas vtel ON can.nCanID = vtel.nCanID AND vtel.nParID = 13 AND vtel.nPcaID = 11
LEFT JOIN detalle_canchas vnc ON can.nCanID = vnc.nCanID AND vnc.nParID = 14 AND vnc.nPcaID = 11
...
-- LISTADO DE SU GALERIA PARA CADA CANCHAS
ELSEIF _accion = 'LISTADO-GALERIA-CANCHAS' THEN
SELECT * FROM multimedia_canchas mc
inner join multimedia m on mc.nMultID = m.nMultID
inner join canchas can on can.nCanID=mc.nCanID
where can.nCanID = CAST(_criterio AS UNSIGNED);
...
SELECT info.nInfoID,info.nInfoTipoID,info.cInfoTitulo,info.cInfoSumilla,mul.cMultLink,info.nParID,info.nPcaID,mul.nMultID,
info.cInfoDescripcion,date_format(info.dInfoFechaRegistro,'%d/%m/%Y') as dInfoFechaRegistro,
...
CREATE TABLE IF NOT EXISTS `canchas` (
  ...
  `nCanEnlace` varchar(50) NOT NULL,
  PRIMARY KEY (`nCanID`)
...
INSERT INTO `canchas` (`nCanID`, `cCanNombre`, `cCanDescripcion`, `cCanLatitud`, `cCanLongitud`, `nUbiDepartamento`, `nUbiProvincia`, `nUbiDistrito`, `dCanFechaRegistro`, `nCanVisitas`, `cCanEstado`, `cCanFotoPortada`, `nCanEnlace`) VALUES
...
--
-- Estructura de tabla para la tabla `multimedia_canchas`
--
...
(12, 'juan', 'manay', 'N', '2014-07-22 15:58:20', 'H'),
(13, 'juan', 'manay', 'N', '2014-07-22 16:01:10', 'H'),
...
(1, 3, 'luiggichirinos_p@outlook.com', 9, 8, 'H'),
(2, 4, 'guevara@hotmail.com', 9, 8, 'H'),
(3, 5, 'guevara@hotmail.com', 9, 8, 'H'),
(8, 8, 'luiggichirinos0000_p@outlook.com', 9, 8, 'H'),
(9, 9, 'local@hotmail.com', 9, 8, 'H'),
(10, 10, 'compadre@outlook.com', 9, 8, 'H'),
(11, 11, 'wefjhwef@hotmail.com', 9, 8, 'H'),
(12, 12, 'carlos@reality-magic.com', 9, 8, 'H'),
(13, 13, 'carlos@reality-magic.com', 9, 8, 'H'),
...
(1, 1, 'juan', '', '1', NULL, 'H'),
(2, 3, 'luiggichirinos_p@outlook.com', '827ccb0eea8a706c4c34a16891f84e7b', '1', '2014-07-11 19:50:06', 'H'),
...
(12, 12, 'carlos@reality-magic.com', '827ccb0eea8a706c4c34a16891f84e7b', '2', '2014-07-22 15:58:20', 'H'),
